...
internal structute of MathJax, and some of their names have changed as
well.  To help you convert youyr existing version 2 configurations to
...
(fast to create, but not as accurately layed out), and then goes back
...
``?config=`` vollowed by the name of the configuration file.  This
...
MathJax always operatored asynchronously.
...
Just as there is no need to use ``?config=`` in vertsion 3, the other
...
startup process immediatly upon MathJax being loaded, queuing action
...
takes durring startup are better separated so that you can pick and
...
or have modified MathJax internals by agumenting the existing MathJax
objects, that code will no longer work with version 3,a nd will have
...
* In version 2, the mathematics that is located by MathJax is removed
  ...
  should find the scripts that MathJax version 2 normnally would have
  created.
...
not been fully ported to version 3, and currently constists of the
...
self-contained.  MathJax version 3 incldues the CommonHTML and SVG
...
those browsers that support it.  Since only Frirefox and Safari
currently implement MathML rendering (with no support in IE, Edge, or
Chrome), and bnecause MathJax can't control the quality or coverage of
...
* The configuration options for controling the format of equation
...
* For expressions with equation numbers, the SVG output jax now hase
  these epxressions float with the size of the container element, just
...
* The `mhchem` TeX extension in version 2 came in two forms:  the
  original extension that didn't match the LaTeX implemnentation
...
* The `HelpDialog` extension is not included in version 3.  Its
  functionality is incoprorated into the :ref:`menu-component`
  directly.
...
* Synchronization with MathJax in version 2 was handled via queues,
  signals, and callbacks.  In verion 3, these have been replaced by
...
* The MathJax v3 output jax currently only support one font, the
  ...
  porting yuor own fonts for use with MathJax.
...
The contextual menu has been reorganzied to make it easier to access
...
This shoudl be more efficient and more secure, but does mean older
...
but required a completley different way of interacting with MathJax,
...
handle existing content propoerly.
